# #!/bin/bash
# # SPDX-FileCopyrightText: Copyright (c) 1993-2025 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# # SPDX-License-Identifier: Apache-2.0

# # Compare Adaptive vs Fixed compression on a benchmark
# #
# # This script runs both fixed and adaptive compression with the same overall
# # compression budget to compare their effectiveness.
# #
# # Usage:
# #   ./evaluate_adaptive_vs_fixed.sh <policy_checkpoint> <c_target> <dataset> <data_dir> [base_press] [model]
# #
# # Examples:
# #   ./evaluate_adaptive_vs_fixed.sh ckpts/policy/policy_epoch10.pt 0.5 ruler 4096
# #   ./evaluate_adaptive_vs_fixed.sh ckpts/policy/policy_epoch10.pt 0.6 longbench null snapkv

# set -e

# # Parse arguments
# POLICY_CKPT=${1:-"/data/evictionKV/ckpts/policy_qwen/policy_qwen_epoch10.pt"}
# C_TARGET=${2:-0.2}
# DATASET=${3:-"ruler"}
# DATA_DIR=${4:-"4096"}
# BASE_PRESS=${5:-"snapkv"}
# # MODEL=${6:-"meta-llama/Llama-2-7b-hf"}
# MODEL=${6:-"Qwen/Qwen2.5-7B-Instruct"}
# # Calculate compression_ratio from c_target
# COMPRESSION_RATIO=$(python3 -c "print(1.0 - $C_TARGET)")

# echo "============================================"
# echo "Adaptive vs Fixed Compression Comparison"
# echo "============================================"
# echo "Dataset: $DATASET"
# echo "Data dir: $DATA_DIR"
# echo "Model: $MODEL"
# echo "Base press: $BASE_PRESS"
# echo "Target retention (c_target): $C_TARGET"
# echo "Compression ratio: $COMPRESSION_RATIO"
# echo "============================================"
# echo ""

# # Check if policy checkpoint exists
# if [ ! -f "$POLICY_CKPT" ]; then
#     echo "ERROR: Policy checkpoint not found at: $POLICY_CKPT"
#     exit 1
# fi

# # Baseline - No compression
# echo "--------------------------------------------"
# echo "1. Running BASELINE (no compression)..."
# echo "--------------------------------------------"
# cmd_baseline="python evaluate.py \
#     --dataset $DATASET \
#     --model $MODEL \
#     --press_name no_press \
#     --device cuda:7"

# if [ "$DATA_DIR" != "null" ] && [ ! -z "$DATA_DIR" ]; then
#     cmd_baseline="$cmd_baseline --data_dir $DATA_DIR"
# fi

# echo "Running: $cmd_baseline"
# eval $cmd_baseline
# echo ""

# # Fixed compression
# echo "--------------------------------------------"
# echo "2. Running FIXED compression..."
# echo "--------------------------------------------"
# cmd_fixed="python evaluate.py \
#     --dataset $DATASET \
#     --model $MODEL \
#     --press_name $BASE_PRESS \
#     --compression_ratio $COMPRESSION_RATIO\
#     --device cuda:7"

# if [ "$DATA_DIR" != "null" ] && [ ! -z "$DATA_DIR" ]; then
#     cmd_fixed="$cmd_fixed --data_dir $DATA_DIR"
# fi

# echo "Running: $cmd_fixed"
# eval $cmd_fixed
# echo ""

# # Adaptive compression
# echo "--------------------------------------------"
# echo "3. Running ADAPTIVE compression..."
# echo "--------------------------------------------"
# ADAPTIVE_PRESS="adaptive_${BASE_PRESS}"

# cmd_adaptive="python evaluate.py \
#     --dataset $DATASET \
#     --model $MODEL \
#     --press_name $ADAPTIVE_PRESS \
#     --policy_checkpoint $POLICY_CKPT \
#     --c_target $C_TARGET \
#     --min_tokens_per_head 1.0\
#     --device cuda:7"

# if [ "$DATA_DIR" != "null" ] && [ ! -z "$DATA_DIR" ]; then
#     cmd_adaptive="$cmd_adaptive --data_dir $DATA_DIR"
# fi

# echo "Running: $cmd_adaptive"
# eval $cmd_adaptive
# echo ""

# echo "============================================"
# echo "All evaluations completed!"
# echo "============================================"
# echo ""
# echo "Results summary:"
# echo "  1. Baseline (no compression): results/*__no_press__*/"
# echo "  2. Fixed compression: results/*__${BASE_PRESS}__${COMPRESSION_RATIO}__*/"
# echo "  3. Adaptive compression: results/*__${ADAPTIVE_PRESS}__*__c_target${C_TARGET}__*/"
# echo ""
# echo "To compare metrics:"
# echo "  cat results/*__no_press__*/metrics.json"
# echo "  cat results/*__${BASE_PRESS}__*/metrics.json"
# echo "  cat results/*__${ADAPTIVE_PRESS}__*/metrics.json"

#!/bin/bash
# SPDX-FileCopyrightText: Copyright (c) 1993-2025 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0

# Compare Adaptive vs Fixed compression on RULER 4k & 8k (no baseline)
#
# Usage:
#   ./evaluate_ruler_4k_8k_snapkv.sh [policy_checkpoint] [c_target] [base_press] [model] [device]
#
# Examples:
#   ./evaluate_ruler_4k_8k_snapkv.sh
#   ./evaluate_ruler_4k_8k_snapkv.sh ckpts/policy/policy_epoch10.pt 0.2 snapkv Qwen/Qwen2.5-7B-Instruct cuda:7

set -e

########################
# 参数解析
########################
POLICY_CKPT=${1:-"/data/evictionKV/ckpts/policy_llama3.1-8B/policy_epoch1.pt"}
C_TARGET=${2:-0.2}
BASE_PRESS=${3:-"snapkv"}
MODEL=${4:-"meta-llama/Llama-3.1-8B-Instruct"}
DEVICE=${5:-"cuda:7"}

DATASET="ruler"

# 和原脚本一致：从 c_target 计算 compression_ratio
COMPRESSION_RATIO=$(python3 -c "print(1.0 - $C_TARGET)")

ADAPTIVE_PRESS="adaptive_${BASE_PRESS}"

echo "============================================"
echo "Adaptive vs Fixed Compression on RULER 4k & 8k"
echo "============================================"
echo "Dataset:        $DATASET"
echo "Model:          $MODEL"
echo "Device:         $DEVICE"
echo "Base press:     $BASE_PRESS"
echo "Adaptive press: $ADAPTIVE_PRESS"
echo "c_target:       $C_TARGET"
echo "compression_ratio (fixed): $COMPRESSION_RATIO"
echo "============================================"
echo ""

# 检查策略 checkpoint
if [ ! -f "$POLICY_CKPT" ]; then
    echo "ERROR: Policy checkpoint not found at: $POLICY_CKPT"
    exit 1
fi

############################################
# 循环跑 ruler 4k (4096) 和 ruler 8k (8192)
############################################

for DATA_DIR in 8192 16384 ; do
    echo "--------------------------------------------"
    echo "Running FIXED compression on RULER ${DATA_DIR}..."
    echo "--------------------------------------------"

    cmd_fixed="python evaluate.py \
        --dataset $DATASET \
        --model $MODEL \
        --press_name $BASE_PRESS \
        --compression_ratio $COMPRESSION_RATIO \
        --data_dir $DATA_DIR \
        --device $DEVICE"

    echo "Running: $cmd_fixed"
    eval $cmd_fixed
    echo ""

    echo "--------------------------------------------"
    echo "Running ADAPTIVE compression on RULER ${DATA_DIR}..."
    echo "--------------------------------------------"

    cmd_adaptive="python evaluate.py \
        --dataset $DATASET \
        --model $MODEL \
        --press_name $ADAPTIVE_PRESS \
        --policy_checkpoint $POLICY_CKPT \
        --c_target $C_TARGET \
        --min_tokens_per_head 1.0 \
        --data_dir $DATA_DIR \
        --device $DEVICE"

    echo "Running: $cmd_adaptive"
    eval $cmd_adaptive
    echo ""
done

echo "============================================"
echo "All evaluations completed (RULER 4k & 8k)!"
echo "============================================"
echo ""
echo "Results summary (pattern only, 按你们实际生成路径为准):"
echo "  * Fixed (snapkv):        results/*__${BASE_PRESS}__${COMPRESSION_RATIO}__*/"
echo "  * Adaptive (adaptive_*): results/*__${ADAPTIVE_PRESS}__*__c_target${C_TARGET}__*/"
echo ""
echo "To compare metrics (示例):"
echo "  # RULER 4k"
echo "  cat results/*ruler*4096*__${BASE_PRESS}__${COMPRESSION_RATIO}__*/metrics.json"
echo "  cat results/*ruler*4096*__${ADAPTIVE_PRESS}__*__c_target${C_TARGET}__*/metrics.json"
echo ""
echo "  # RULER 8k"
echo "  cat results/*ruler*8192*__${BASE_PRESS}__${COMPRESSION_RATIO}__*/metrics.json"
echo "  cat results/*ruler*8192*__${ADAPTIVE_PRESS}__*__c_target${C_TARGET}__*/metrics.json"
