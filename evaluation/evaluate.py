...
# Add parent directory to path to use local kvpress if available
...
class EvaluationConfig:
    ...
    # Adaptive per-head compression parameters
    ...
    def __post_init__(self):
        ...
        # Validate press
        # Adaptive presses are dynamically created, so skip registry check for them
        if not self.press_name.startswith("adaptive_"):
            assert self.press_name in PRESS_REGISTRY, (
                f"Press '{self.press_name}' not found in PRESS_REGISTRY. "
                f"Available presses: {list(PRESS_REGISTRY.keys())}. "
                f"Note: For adaptive compression, use 'adaptive_<base_press_name>' format."
            )
        ...
        # Validate adaptive per-head compression parameters
        if self.press_name.startswith("adaptive_"):
            assert self.policy_checkpoint is not None, (
                f"policy_checkpoint must be set for adaptive press '{self.press_name}'"
            )
            # Use c_target if provided, otherwise derive from compression_ratio
            if self.c_target is None:
                self.c_target = 1.0 - self.compression_ratio
                logger.info(f"c_target not set, derived from compression_ratio: {self.c_target}")
            assert 0.0 < self.c_target <= 1.0, f"c_target must be between 0.0 and 1.0, got {self.c_target}"
            assert self.min_tokens_per_head >= 0, (
                f"min_tokens_per_head must be non-negative, got {self.min_tokens_per_head}"
            )
    ...
    def get_results_dir(self, output_dir: Path) -> Path:
        ...
        if self.c_target is not None and self.press_name.startswith("adaptive_"):
            components.append(f"c_target{self.c_target:.2f}")
        if self.press_name.startswith("adaptive_") and self.min_tokens_per_head != 1.0:
            components.append(f"min_tokens{self.min_tokens_per_head:.1f}")
...
class EvaluationRunner:
    ...
    def _setup_press(self):
        ...
        # Handle adaptive per-head compression
        if press_name.startswith("adaptive_"):
            # Extract base press name (e.g., "adaptive_observed_attention" -> "observed_attention")
            base_press_name = press_name[len("adaptive_"):]
            
            # Check if base press exists in registry
            if base_press_name not in PRESS_REGISTRY:
                available_presses = [k for k in PRESS_REGISTRY.keys() if k != "no_press"]
                raise ValueError(
                    f"Base press '{base_press_name}' not found in PRESS_REGISTRY. "
                    f"Available base presses: {available_presses}. "
                    f"Use format: 'adaptive_<base_press_name>' (e.g., 'adaptive_snapkv', 'adaptive_observed_attention')"
                )
            
            # Get base press from registry
            base_press = PRESS_REGISTRY[base_press_name]
            
            if base_press is None:
                raise ValueError(
                    f"Cannot use AdaptivePerHeadPress with 'no_press'. "
                    f"Please specify a valid base press."
                )
            
            # Create a new instance of base_press to avoid shared state issues
            # For dataclass instances, we can use dataclasses.replace or create a new instance
            import copy
            try:
                # Try to create a deep copy to avoid shared state
                base_press_copy = copy.deepcopy(base_press)
            except Exception:
                # If deepcopy fails (e.g., for complex objects), use the original
                # This is safe for single-threaded evaluation
                logger.warning(
                    f"Could not deepcopy base press '{base_press_name}', using original instance. "
                    f"This is safe for single-threaded evaluation."
                )
                base_press_copy = base_press
            
            # Set compression_ratio to 0 for base_press since AdaptivePerHeadPress manages compression itself
            # This ensures base_press's compress method won't interfere
            if hasattr(base_press_copy, "compression_ratio"):
                base_press_copy.compression_ratio = 0.0
                logger.debug(f"Set base press '{base_press_name}' compression_ratio to 0.0 (managed by AdaptivePerHeadPress)")
            
            # Create AdaptivePerHeadPress wrapper
            logger.info(
                f"Creating AdaptivePerHeadPress with base press: {base_press_name}, "
                f"policy_checkpoint: {self.config.policy_checkpoint}, "
                f"c_target: {self.config.c_target}, "
                f"min_tokens_per_head: {self.config.min_tokens_per_head}"
            )
            
            press = AdaptivePerHeadPress(
                press=base_press_copy,
                policy_checkpoint=self.config.policy_checkpoint,
                c_target=self.config.c_target,
                min_tokens_per_head=self.config.min_tokens_per_head,
            )
        else:
        ...
        elif isinstance(press, AdaptivePerHeadPress):
            # AdaptivePerHeadPress uses c_target instead of compression_ratio
            # c_target is already set during initialization, no need to modify
            logger.info(
                f"AdaptivePerHeadPress initialized with c_target={press.c_target}, "
                f"min_tokens_per_head={press.min_tokens_per_head}"
            )
    ...
    def _setup_model_pipeline(self):
        ...
        # Check if press requires eager attention (ObservedAttentionPress or wrapped in AdaptivePerHeadPress)
        needs_eager = isinstance(self.press, ObservedAttentionPress)
        if isinstance(self.press, AdaptivePerHeadPress):
            needs_eager = isinstance(self.press.press, ObservedAttentionPress)
        
        if needs_eager:
            model_kwargs["attn_implementation"] = "eager"
            # For ObservedAttentionPress, we need to manage memory carefully
            # Enable gradient checkpointing to reduce memory usage
            if "use_cache" not in model_kwargs:
                model_kwargs["use_cache"] = True  # We need cache for compression
            logger.info("ObservedAttentionPress (or wrapped) detected, setting attn_implementation to 'eager'.")
            logger.warning(
                "ObservedAttentionPress requires storing full attention matrices, which can be memory-intensive "
                "for long sequences (4096 tokens ≈ 2GB per layer). If OOM occurs, try:\n"
                "  1. Use a smaller model (e.g., Llama-3.2-1B instead of 7B)\n"
                "  2. Reduce sequence length (e.g., data_dir='2048' instead of '4096')\n"
                "  3. Use fraction < 1.0 for testing\n"
    ...
    def _run_inference(self):
        ...
        if isinstance(self.press, DecodingPress):
            ...
            for index, row in tqdm(self.df.iterrows(), total=len(self.df), desc="Running Inference"):
                ...
                # Aggressive memory cleanup for ObservedAttentionPress
                if isinstance(self.press, ObservedAttentionPress) or (
                    isinstance(self.press, AdaptivePerHeadPress) and isinstance(self.press.press, ObservedAttentionPress)
                ):
